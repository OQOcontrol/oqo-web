      ...
      <main className="">
        <section id="billboard" className="md:h-screen mb-24 md:mb-36 relative">
          <div className="hidden md:grid grid-cols-2 absolute -rotate-90 top-full origin-top-left	font-saansMono text-grey" style={{width: "100vh", left: "95%"}}>
            <div className="text-xs">{t['The OQO (o-cu-o) is a device with wireless sensors that allows for real-time modulation of sounds, lights, and visuals.']}</div>
          ...
          <h1 className="block md:absolute text-2xl md:text-4xl md:w-8/12 top-0 font-abc">{t['The OQO (o-cu-o) is a device with wireless sensors that allows for real-time modulation of sounds, lights, and visuals.']}</h1>
        </section>
        ...
        <section id="about" className="mb-24 md:mb-36">
          <div className="md:grid grid-cols-2">
            <div>
              <h2 className="mb-2 text-xs font-saansMono">{t['About']}</h2>
            </div>
            <div>
              <p className="mb-4 text-lg">{t['OQO is not just a product but an ever-evolving artistic project. Its purpose is to break the traditional boundaries of audiovisual control, exploring new forms of interaction between sound, light, and movement. Through the use of wireless sensors and technological innovation, OQO offers an experimental and creative approach that opens the door to immersive sensory experiences.']}</p>
              <p className="mb-4 text-lg">{t['It’s not just about technology, but about how it can redefine the connection between artists and audiences, creating an audiovisual experience that challenges conventional approaches to live performances.']}</p>
            </div>
          </div>
        </section>
        ...
        <h2 className="mb-24 md:mb-36 text-2xl md:text-4xl font-abc">{t['Leveraging the advances in new technologies and the development of more efficient sensors, OQO introduces a new approach to audiovisual control.']}</h2>
          <div className="md:grid grid-cols-2">
            <h2 className="mb-2 text-xs font-saansMono">{t['The Project']}</h2>
            <div className="mb-24 md:mb-36">
              <p className="mb-4 text-lg">{t['Developed throughout 2023, this innovative design incorporates sensors held by magnets in a vertical and transparent structure.']}</p>
              <p className="mb-4 text-lg">{t['Each sensor can be programmed to control sound effects, lights, or visuals, and is integrated into a knob with a visual and illuminated position indicator for easier handling. Assigned parameters can be adjusted by turning the knob on its axis or moving it in the air, allowing for direct interaction with the performer’s body.']}</p>
              <p className="mb-4 text-lg">{t['A distinctive feature of OQO is the ability to connect the knobs to each other using magnets, opening up new possibilities for live control. This allows multiple parameters to be adjusted simultaneously with each hand, offering a novel and creative form of interaction.']}</p>
              <p className="mb-8 text-lg">{t['The vertical design of the module promotes a more ergonomic posture, and being completely transparent, it creates no visual barrier between the performer and the audience, allowing for a fluid and direct connection during the performance. All information generated by the knobs is transmitted wirelessly to a computer, synthesizer, or other MIDI-compatible devices.']}</p>
          ...
          <h2 className="mb-24 md:mb-36 text-2xl md:text-4xl font-abc">{t['It’s not just about technology, but about how it can redefine the connection between artists and audiences, creating an audiovisual experience that challenges conventional approaches to live performances.']}</h2>  
        </section>
        <section id="andres">
        <div className="md:grid grid-cols-2">
            <div>
              <h2 className="mb-2 text-xs font-saansMono">{t['About Andrés']}</h2>
            </div>
            <div>
              <p className="mb-4 text-lg">{t['Born in Argentina and living in Barcelona since 2018. Throughout his career, he has explored the fusion of sounds and technology, using sensors and experimental tools to develop innovative concepts. His focus is always on discovering new ways of sound execution and modulation.']}</p>
            </div>
          </div>
        </section>
        <footer>
